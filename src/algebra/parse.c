...
Boolean is_juxtaposed(struct EquationObject self, struct EquationObject last)
{
    ...
    // (3)x
    if (cur_pi_letter && last.type == BLOCK_END)
    {
        return TRUE;
    }
    // x(2+3)
    ...
    // xy
    if (cur_pi_letter && last_pi_letter)
    {
        return TRUE;
    }
    // Almost all the functions
    if (is_function(self.type) && self.type != EXP &&
        (last.type == NUMBER || last.type == BLOCK_END || last_pi_letter))
    {
        return TRUE;
    }
    return FALSE;
                       ...
                       struct InputVar* buffer, int num_args)
{
    ...
    while (i < new_len)
    {
        if (out_buffer[i].type == PI_VAL)
        {
            out_buffer[i].type = NUMBER;
            out_buffer[i].value.number = PI;
        }
        i++;
    }
