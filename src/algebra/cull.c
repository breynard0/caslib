...
int cull_the_useless(struct EquationObject *expression, int length) {
  ...
  while (i < new_len) {
    ...
    if (buf[i].type == NUMBER && buf[i].value.number == 0 &&
      ...
      int before = i - 3;
      if (before >= 0 && (buf[before].type == ADD || buf[before].type == SUB || buf[before].type == MULT || buf[before].type == DIV)) {
        ...
        // Remove sign
        int after = i + 1;
        if (buf[after].type == ADD || buf[after].type == SUB || buf[after].type == MULT || buf[after].type == DIV) {
